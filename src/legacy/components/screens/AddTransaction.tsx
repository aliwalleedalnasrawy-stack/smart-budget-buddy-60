...
export const AddTransaction = ({ categories, currencySymbol, pots, onAdd, onAddPot, onDeletePot, onDone }: Props) => {
  ...
  // For saving, the implicit category is the savings_cat
  const savingCat = categories.find(c => c.id === 'savings_cat');
  const selCat = isSaving ? savingCat : categories.find(c => c.id === category);
  const selPot = pots.find(p => p.id === potId);

  const canSave = parseFloat(amount) > 0 && (isSaving ? !!potId : !!category);

  const submit = () => {
    const n = parseFloat(amount);
    if (!n || n <= 0) return;
    if (isSaving && !potId) return;
    if (!isSaving && !category) return;
    onAdd({
      name: name.trim() || undefined,
      amount: n,
      type,
      category: isSaving ? (savingCat?.id || 'savings_cat') : category,
      date,
      note: note || undefined,
      pot: isSaving ? potId : undefined,
    });
    onDone();
  };
  ...
  return (
    <div className="pb-28 pt-4 px-4 max-w-lg mx-auto w-full overflow-x-hidden">
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-xl font-black text-white mb-6">إضافة معاملة</h2>

        <div className="rounded-3xl p-4 sm:p-6 space-y-5"
            ...
            <div className="grid grid-cols-3 gap-2">
              {typeOpts.map(({ v, l, c }) => (
                <motion.button key={v} whileTap={{ scale: 0.94 }}
                  onClick={() => { setType(v); setCat(''); setPotId(''); setCatOpen(false); }}
          ...
          <div>
            ...
            <div className="flex items-center gap-3 rounded-2xl px-4 py-4"
              ...
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                ...
                className="flex-1 min-w-0 bg-transparent text-white text-2xl sm:text-3xl font-black outline-none text-right"
                style={{ direction: 'ltr' }}/>
              <span className="text-lg sm:text-xl font-bold shrink-0" style={{ color: '#D4A017' }}>{currencySymbol}</span>
            </div>
          </div>

          {/* Category / Saving Pot — unified field */}
          <div>
            <label className="text-xs font-semibold mb-2 block flex items-center gap-1.5" style={{ color: isSaving ? '#D4A017' : '#9CA3AF' }}>
              {isSaving ? <><PiggyBank size={13}/> صندوق الادخار</> : 'الفئة'}
            </label>
            <button onClick={() => setCatOpen(p => !p)}
              className="w-full flex items-center justify-between px-4 py-4 rounded-2xl"
              style={{
                background: 'rgba(2,6,23,0.7)',
                border: (isSaving ? !!potId : !!category) ? '1.5px solid rgba(212,160,23,0.3)' : '1.5px solid rgba(255,255,255,0.08)',
              }}>
              <ChevronDown size={15} style={{ color: '#4B5563', transform: catOpen ? 'rotate(180deg)' : undefined, transition: 'transform .2s' }}/>
              {isSaving ? (
                selPot ? (
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-sm font-semibold text-white truncate">{selPot.name}</span>
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
                      style={{ background: 'rgba(212,160,23,0.2)' }}>
                      <PiggyBank size={15} color="#D4A017"/>
                    </div>
                  </div>
                ) : (
                  <span className="text-sm" style={{ color: '#374151' }}>اختر صندوق الادخار</span>
                )
              ) : selCat ? (
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-sm font-semibold text-white truncate">{selCat.name}</span>
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                {isSaving ? (
                  <div className="p-3 space-y-2">
                    {/* Create new pot button at the top */}
                    <button onClick={() => setShowNewPot(p => !p)}
                      ...
                      style={{ background: 'rgba(212,160,23,0.12)', color: '#D4A017', border: '1px dashed rgba(212,160,23,0.4)' }}>
                      <Plus size={14}/> إنشاء صندوق جديد
                    </button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {pots.map(p => (
                          <div key={p.id} className="relative">
                            <button onClick={() => { setPotId(potId === p.id ? '' : p.id); setCatOpen(false); }}
                              className="w-full flex items-center gap-2 p-3 rounded-xl text-right"
                              style={{
                                background: potId === p.id ? 'rgba(212,160,23,0.15)' : 'rgba(2,6,23,0.5)',
                                border: potId === p.id ? '1.5px solid #D4A017' : '1px solid rgba(255,255,255,0.06)',
                              }}>
                              <PiggyBank size={14} style={{ color: potId === p.id ? '#D4A017' : '#6B7280' }}/>
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-bold truncate"
                                  style={{ color: potId === p.id ? '#D4A017' : '#E5E7EB' }}>{p.name}</p>
                                {p.target && (
                            ...
                            <button onClick={() => { onDeletePot(p.id); if (potId === p.id) setPotId(''); }}
                  ...
                  <div className="p-3 grid grid-cols-3 gap-2">
                    {availCats.map(cat => (
                      <motion.button key={cat.id} whileTap={{ scale: 0.92 }}
                        ...
                        <span className="text-[10px] text-center font-medium break-words"
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={submit} disabled={!canSave}
            className="w-full py-4 rounded-2xl font-bold text-base sm:text-lg flex items-center justify-center gap-2"
            style={{
